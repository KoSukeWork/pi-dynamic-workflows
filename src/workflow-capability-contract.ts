...
const capabilities: readonly CapabilityDescriptor[] = [
  runtimeGlobal("agent", {
    ...
    constraints: [
      ...
      "failed attempts remove only their own shared-store writes; equal-valued sibling writes survive and later rollbacks cannot restore discarded writes",
      ...
      "journaled structured results are independent snapshots; mutating returned values cannot change later replays",
      "versioned shared-store deltas preserve the last successful write per key across parallel replay; legacy deltas without versions retain call-order replay",
      ...
      "worktree isolation failure stops the agent without running in the shared directory; changed or committed worktrees are retained and their paths and branches are logged for inspection",
      "resume reuses retained worktrees for unchanged call prefixes, including nested workflows with disabled result caches, after verifying workflow ownership, repository, branch and original base; changed upstream or agent calls get a fresh worktree",
      "worktree identities use run IDs, frame-local call indices, and input prefixes; display labels can change without restarting retained work",
    ],
    evidence: [
      "tests/workflow-runtime.test.ts",
      "tests/shared-store-rollback.test.ts",
      "tests/shared-store-replay.test.ts",
      ...
      "tests/worktree-identity.test.ts",
    ],
  }),
  ...
  runtimeGlobal("workflow", {
    ...
    constraints: [
      ...
      "nested workflows replay their own namespaced journal entries only while the upstream prefix matches; a child miss invalidates following parent and sibling calls",
      "nested identities include the child script, arguments, and sorted agent-definition snapshot; changes invalidate downstream caches and worktrees independently of completion order",
      "a child's private cache and worktree scope also includes the enclosing script and initial arguments; enclosing edits conservatively invalidate that child without changing the parent's own positional worktree prefix",
      "child and enclosing arguments must contain serializable data; functions and promises are rejected before child agents start",
      "calls made while a child is pending run live because its unchanged prefix is not yet proven",
      "a parent miss also disables subsequent agent and checkpoint replay in an already-running child",
    ],
    evidence: [
      "tests/workflow-saved.test.ts",
      "tests/shared-store.test.ts",
      "tests/nested-resume.test.ts",
      "tests/parent-resume.test.ts",
    ],
  }),
  ...
  runtimeGlobal("checkpoint", {
    ...
    constraints: [
      ...
      "structured checkpoint answers are snapshotted independently of returned values on capture and replay",
    ],
    evidence: ["tests/checkpoint.test.ts", "tests/workflow-journal.test.ts"],
  }),
  ...
  ]),
  {
    ...toolInput("args", "args?: unknown", [
      "managed runs snapshot initial arguments and explicit resume overrides; each execution gets a separate mutable copy",
      "managed arguments must be cloneable; use JSON-compatible values for disk persistence",
    ]),
    enforcementOwner: "WorkflowManager input snapshots and runWorkflow context assembly",
    behaviorEvidence: ["tests/workflow-tool.test.ts", "tests/workflow-manager-args.test.ts"],
  },
