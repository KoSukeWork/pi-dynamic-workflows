    ...
    if (!ready) {
      ...
      const attempt = loadWithRetry(load, (error) => {
        ...
        if (typeof mod.default !== "function") {
          const cause = firstLoadError ? `; the first load attempt failed with: ${firstLoadError}` : "";
          throw new Error(`Extension runtime does not export a factory (default is ${typeof mod.default})${cause}`);
        }
        factoryStarted = true;
        let result: unknown;
        try {
